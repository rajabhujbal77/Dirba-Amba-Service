...
import { depotsApi, packagesApi, depotPricingApi, bookingsApi, seasonApi, contactsApi, receiptsApi, creditApi } from '../utils/api';
...
export default function NewBooking({ onNavigate }: NewBookingProps) {
  ...
  const [customerPricing, setCustomerPricing] = useState<any[]>([]); // Customer-specific pricing
    ...
    try {
      ...
      if (isOnline) {
        ...
        try {
          ...
          // Save custom pricing for this customer (if any custom prices were used)
          try {
            const customPricesToSave: { packageId: string; depotId: string; discountedPrice: number }[] = [];

            for (const receiver of formData.receivers) {
              for (const pkg of receiver.packages) {
                // Get standard price for comparison
                const standardPriceEntry = pricing.find(
                  (p: any) => p.packageId === pkg.packageId && p.depotId === formData.destinationDepotId
                );
                const standardPrice = standardPriceEntry?.price || packages.find((p: any) => p.id === pkg.packageId)?.basePrice || 0;

                // Only save if price differs from standard
                if (pkg.price !== standardPrice) {
                  customPricesToSave.push({
                    packageId: pkg.packageId,
                    depotId: formData.destinationDepotId,
            ...
            if (customPricesToSave.length > 0 && formData.senderPhone) {
              await creditApi.setCustomerPricing(formData.senderPhone, customPricesToSave);
              console.log('[NewBooking] Saved custom pricing for customer:', formData.senderPhone, customPricesToSave);
            }
          } catch (pricingError) {
            console.error('[NewBooking] Failed to save custom pricing:', pricingError);
            // Don't fail the booking if pricing save fails
          }
      ...
      >
        ...
        {currentStep === 3 && (
          <Step3ReceiversPackages
            ...
            setCustomerPricing={setCustomerPricing}
            onNext={nextStep}
            onPrev={prevStep}
          />
        )}
...
// ============================================================================
function Step3ReceiversPackages({ formData, setFormData, packages, pricing, customerPricing, setCustomerPricing, onNext, onPrev }: any) {
  const validatePhone = (phone: string) => /^[6-9]\d{9}$/.test(phone);
  ...
  // Get default price (from customer pricing, then depot pricing, then base)
  const getDefaultPrice = (packageId: string) => {
    // First check customer-specific pricing
    const customerPrice = customerPricing.find(
      (p: any) => p.packageId === packageId && p.depotId === formData.destinationDepotId
    );
    if (customerPrice) return customerPrice.discountedPrice;

    // Then check depot pricing
    const priceEntry = pricing.find(
      (p: any) => p.packageId === packageId && p.depotId === formData.destinationDepotId
    );
    if (priceEntry) return priceEntry.price;

    // Finally use base price
    return packages.find((p: any) => p.id === packageId)?.basePrice || 0;
  };

  // Get the standard depot price (for comparison)
  const getStandardPrice = (packageId: string) => {
    const priceEntry = pricing.find(
      (p: any) => p.packageId === packageId && p.depotId === formData.destinationDepotId
    );
  ...
  const updateReceiverPackage = (receiverIndex: number, packageId: string, quantity: number, customPrice?: number) => {
    const newReceivers = [...formData.receivers];
    const receiver = newReceivers[receiverIndex];
    const pkg = packages.find((p: any) => p.id === packageId);

    // Use custom price if provided, otherwise get from existing package or default
    const existingPkg = receiver.packages.find((p: any) => p.packageId === packageId);
    const price = customPrice !== undefined ? customPrice : (existingPkg?.price || getDefaultPrice(packageId));
    ...
    if (quantity > 0) {
      ...
      if (existingIndex >= 0) {
        receiver.packages[existingIndex] = { ...receiver.packages[existingIndex], ...pkgData };
      } else {
  ...
  // Update price for a package
  const updateReceiverPackagePrice = (receiverIndex: number, packageId: string, newPrice: number) => {
    const newReceivers = [...formData.receivers];
    const receiver = newReceivers[receiverIndex];
    const existingIndex = receiver.packages.findIndex((p: any) => p.packageId === packageId);

    if (existingIndex >= 0) {
      receiver.packages[existingIndex].price = newPrice;
      setFormData({ ...formData, receivers: newReceivers });
    }
  };

  // Get current price for a package (from receiver's selected packages or default)
  const getReceiverPackagePrice = (receiverIndex: number, packageId: string) => {
    const pkg = formData.receivers[receiverIndex]?.packages?.find((p: any) => p.packageId === packageId);
    return pkg?.price ?? getDefaultPrice(packageId);
  };
        ...
        <div key={receiverIndex} className="p-6 border-2 border-gray-200 rounded-lg space-y-4 bg-gray-50">
          ...
          <div>
            ...
            {customerPricing.length > 0 && (
              <p className="text-xs text-orange-600 mb-2">💰 Special pricing applied for this customer</p>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {packages.map((pkg: any) => {
                const currentPrice = getReceiverPackagePrice(receiverIndex, pkg.id);
                const standardPrice = getStandardPrice(pkg.id);
                ...
                const isCustomPrice = quantity > 0 && currentPrice !== standardPrice;
                return (
                  <div key={pkg.id} className={`p-3 bg-white border rounded-lg hover:border-orange-300 transition-colors ${isOther && quantity > 0 ? 'border-yellow-400 col-span-full' : isCustomPrice ? 'border-orange-400' : 'border-gray-200'}`}>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (quantity > 0) {
                            ...
                            // If quantity is 0, set quantity to 1 with the new price
                            updateReceiverPackage(receiverIndex, pkg.id, 1, newPrice);
                          }
                        }}
                        className={`w-14 px-1 py-0.5 text-center text-xs border rounded focus:ring-1 focus:ring-orange-500 ${isCustomPrice ? 'border-orange-400 bg-orange-50 font-medium' : 'border-gray-200'}`}
                      />
                      <span className="text-xs text-gray-500">/unit</span>
                      {isCustomPrice && (
                        <span className="text-xs text-green-600 font-medium" title={`Standard: ₹${standardPrice}`}>
                          {currentPrice < standardPrice ? '-' : '+'}₹{Math.abs(standardPrice - currentPrice)}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {quantity > 0 && (
                        <span className="text-xs font-medium text-green-600">
                          ₹{(quantity * currentPrice).toFixed(0)}
                        </span>
                      )}
                    </div>
